...
interface ExtratoOrganizacaoTabProps {
  tenantId: number;
  tenantName: string;
}

export default function ExtratoOrganizacaoTab({ tenantId, tenantName }: ExtratoOrganizacaoTabProps) {
  ...
  useEffect(() => {
    fetchTransactions();
  }, [tenantId, filters, dateRange, pagination.offset]);
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const response = await ledgerApi.listTransactions(tenantId, {
   ...
   * DEPOSIT usa net_amount ou gross_amount
   * WITHDRAWAL usa amount ou total_amount
   */
  const getTransactionAmount = (tx: any): string => {
    ...
    // Fallback: tenta pegar de qualquer campo de valor
    return tx.metadata.amount || tx.metadata.net_amount || tx.metadata.gross_amount || tx.metadata.total_amount || "0";
  };

  /**
   * Encontra o posting relacionado a uma conta específica
   */
  const getPostingForAccount = (tx: any, accountId?: string) => {
    if (!tx.postings || !Array.isArray(tx.postings)) return null;
    if (!accountId) return tx.postings[0]; // Retorna o primeiro se não especificar conta
    return tx.postings.find((p: any) => String(p.accountId) === String(accountId));
  };
  ...
  const exportToCSV = () => {
    ...
    link.download = `extrato-${tenantName}-${format(new Date(), "yyyy-MM-dd")}.csv`;
    link.click();
