...
interface ContasOrganizacaoTabProps {
  tenantId: number;
}

export default function ContasOrganizacaoTab({ tenantId }: ContasOrganizacaoTabProps) {
  ...
  const handleViewDetails = (accountId: number) => {
    navigate(`/contas-organizacoes/conta/${accountId}?tenantId=${tenantId}`);
  };
              ...
              <tbody>
                {accounts.map((account, index) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      <span className="text-sm">{account.accountPurpose || "N/A"}</span>
                    </td>
                    <td className="p-3">
