...
import { Search, RefreshCcw, Download, Loader2, Calendar as CalendarIcon, X, ChevronDown, ChevronUp, Copy } from "lucide-react";
...
export default function ExtratoPage() {
  ...
  }, [selectedTenantId, filters, dateRange, pagination.offset]);
  ...
  const fetchTenants = async () => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        setFilters({ ...filters, tenantId: response.data[0].id.toString() });
      }
    } catch (error: any) {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const response = await ledgerApi.listTransactions(selectedTenantId, {
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2 block">
              Selecionar Organização (OBRIGATÓRIO)
            </label>
            <Select
      ...
      {!selectedTenantId ? (
        <Card className="p-12 text-center">
          <p className="text-muted-foreground">
            Selecione uma organização para ver o extrato
          </p>
        </Card>
      ) : (
        <>
