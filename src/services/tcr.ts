...
/** Mapeia resposta qtran ou v2 para formato da UI */
function mapearTransacaoParaUI(data: any, endtoend: string): TransacaoVerificadaUI {
  // qtran: amount em centavos; v2: amount em BRL
...
export async function consultarTransacaoPorEndToEndTCR(
  ...
  try {
    ...
    const transacao = mapearTransacaoParaUI(data, endtoendTrim);
 ...
 * Header: X-Corpx-Account-Context: {alias}
 */
export async function consultarTransacaoPorEndToEndV2TCR(
  alias: string,
  endtoend: string
): Promise<VerificacaoTransacaoResult> {
  try {
    ...
    console.error('[TCR-V2-TRANSACTIONS] Erro ao consultar transação:', error);
    return {
      sucesso: false,
      mensagem: `Erro ao consultar transação: ${error.message || 'Erro de conexão'}`,
      permiteOperacao: false
...
export async function transferenciaInternaTCR(
  ...
  try {
    ...
    return data as TransferenciaInternaResponse;
  } catch (error: any) {
    console.error('[TCR-TRANSF-INTERNA] Erro:', error);
    throw error;
  }
}
