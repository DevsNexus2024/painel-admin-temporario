...
export default function TopBarTTF() {
  // ✅ CNPJ da TTF
  const cnpjDefault = "14283885000198"; // CNPJ da TTF
  ...
  const handleRefresh = async () => {
    ...
    try {
      const { consultarSaldoCorpX } = await import('@/services/corpx');
      const resultado = await consultarSaldoCorpX(cnpjDefault);
      
      if (resultado && !resultado.erro) {
        setSaldoData(resultado);
        setLastUpdated(new Date());
      } else {
        setErrorSaldo('Erro ao consultar saldo TTF');
      }
    } catch (error: any) {
      setErrorSaldo(error.message || 'Erro ao consultar saldo');
    } finally {
  ...
  const getSaldoDisplay = () => {
    ...
    if (errorSaldo) return 'Erro ao consultar saldo TTF';
    if (saldoData?.erro) return 'Erro na API';
  ...
  const getContaInfo = () => {
    return (
      <div className="text-xs text-muted-foreground">
        <span>
          TTF SERVICOS DIGITAIS LTDA • BRL • {lastUpdated ? lastUpdated.toLocaleTimeString('pt-BR') : 'Nunca atualizado'}
        </span>
      </div>
    );
  };
    ...
    <div className="sticky top-0 z-30 bg-tcr-dark/95 backdrop-blur-xl border-b border-border h-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-foreground">CorpX TTF → TCR</h1>
              <Badge className="bg-blue-100 text-blue-800 border-blue-200 text-xs font-medium">
                Banking
              </Badge>
              {getStatusIcon()}
            </div>
            <p className="text-muted-foreground text-sm">Central de transferências PIX - TTF</p>
          </div>
        </div>
