...
import { RefreshCcw, Loader2, DollarSign, Wifi, WifiOff, CheckCircle, AlertCircle, FileText, Banknote, Lock, ArrowDownCircle, ArrowUpCircle } from "lucide-react";
...
export default function TopBarBrasilCashOtc() {
  ...
  // Buscar saldo REAL da API BrasilCash com header x-otc-id
  const fetchSaldo = async () => {
    ...
    try {
      ...
      // Buscar saldo usando /api/brasilcash/account/me/balance
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold text-foreground">BrasilCash OTC</h1>
            ...
            <div className="flex flex-col gap-1">
              <p className="text-muted-foreground text-sm">
                Central de transferências PIX OTC {isConnected && '• Atualizações em tempo real'}
              </p>
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                <span className="font-medium">{selectedAccount.name}</span>
                <span>•</span>
                <span className="font-mono">OTC ID: {selectedAccount.otcId}</span>
              </div>
            </div>
          </div>
        </div>
